...
public partial struct PlayerThrowSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        for (int i = 0; i < elementsBuffer.Length; i++)
        {
            ...
            if (element.Type == playerComponentData.CollectibleType && element.Amount <= 0)
            {
                return;
            }
        }
        ...
        Entity elementEntity = state.EntityManager.Instantiate(bulletBufferElementData[(int)playerComponentData.CollectibleType - 1].BulletPrefab); // Switch to the element prefab you want to spawn
        ...
        {
            Amount = -1,
            ElementType = playerComponentData.CollectibleType
        });
